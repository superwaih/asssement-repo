...
const bookDatabase: Book[] = [
  ...
  },
  {
    ...
    mood: ['Angst', 'Reflective'],
  },
  {
    title: 'A Brief History of Time',
    author: 'Stephen Hawking',
    genres: ['Non-fiction', 'Science'],
    mood: ['Curious', 'Awe-inspiring'],
  },
  {
...
const BookRecommendationApp: React.FC = () => {
  ...
  const getRecommendations = async () => {
    ...
    const filteredBooks = bookDatabase.filter(
      (book) =>
    ...
    const sortedBooks = filteredBooks.sort((a, b) => {
      const aRelevance =
        a.genres.filter((genre) => selectedGenres.includes(genre)).length +
        (a.mood.includes(selectedMood) ? 1 : 0);
      const bRelevance =
        b.genres.filter((genre) => selectedGenres.includes(genre)).length +
        (b.mood.includes(selectedMood) ? 1 : 0);
      return bRelevance - aRelevance;
    });

    setRecommendations(sortedBooks.slice(0, 5));
    setIsLoading(false);
  };
      ...
      <div className="mt-8">
        ...
        {isLoading ? (
          ...
          <p className="text-gray-600">
            Select genres and/or mood, then click "Get Recommendations" to see
